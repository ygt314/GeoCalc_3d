...
| | Termux | QPython |
|---|---|---|
| Python | 独立环境（装不动 pywebview 后端） | 自带，QPyPI 装 `sympy`/`bottle` |
| 系统 WebView | ✗（终端调不到） | ✓（Bottle 起服务后自动打开） |
| 后端形态 | 纯 API 的 HTTP 服务 | 纯 API 的 HTTP 服务（同一套 `main.py`） |
| 能干什么 | 前端开发 / 构建 | 直接跑完整应用 |

> 后端 HTTP 化入口即包内 `main.py`（Bottle：静态托管 `ui/` + `/api/<ns>/<method>` + `/api/file/save|load` + 注入 shim）；两个环境共用同一份，只是**显示方式不同**——Termux 手动开浏览器，QPython 自动开 WebView。QPython 包内端口：2D=`8080`、3D=`8081`。
