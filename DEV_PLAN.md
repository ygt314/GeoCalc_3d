...
- [x] **M1 骨架跑通**:后端代码结构 + venv 依赖 + solve 空转(只解表达式相等)
- [x] **M2 DSL 基础**:AB 距离 / vecAB / zA 坐标 / angABC 能解析
- [x] **M3 核心条件**:线线平行垂直、线面、面面、共面、点面距
- [x] **M4 特殊对象**:四面体体积、二面角、平面方程
- [ ] **M5 LaTeX + 前端**:to_raw_latex 美化 + Quasar 前端适配(加 z 输入框)
- [ ] **M6 极值探索**:get_expore(已留 improve_flag,前端适配后补入口)
- [ ] **M7 打包**(可选):Nuitka 打包(参照原版)

> 决策记录(2026-08-03):get_expore 极值探索已实现但标了 [improve_flag] 为实验性,
> 暂不完善 —— 等前端适配完成、有 UI 入口后再回来打磨。
